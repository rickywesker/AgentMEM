...
> 3. **LLM 事实抽取只作为检索索引,不作为返回内容。** 写入时用 gpt-4o-mini 抽取
>    自包含事实句,每条记录指回它的来源对话轮次;检索命中事实后返回的是**原文轮次**,
>    抽取结果本身从不进入答案上下文。最初的实现让事实和原文抢同样的 100 个返回槽位,
>    实测 temporal −17、adversarial −26;改成"事实当键、原文当值"后同一批事实
>    从 60.2 回到 64.4。写入侧唯一的生成式模型调用就是这一处,用的是规则指定的
>    gpt-4o-mini。抽取失败时自动降级为纯原文索引,不影响写入成功。
>
> **合规**
>
